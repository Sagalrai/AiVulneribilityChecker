...
function color(text: string, code: string, bold = false): string {
    return `${bold ? C.bold : ""}${code}${text}${C.reset}`;
}

function green(text: string, bold = false): string {
    return color(text, C.green, bold);
}
function red(text: string, bold = false): string {
    return color(text, C.red, bold);
}
function yellow(text: string, bold = false): string {
    return color(text, C.yellow, bold);
}
function blue(text: string, bold = false): string {
    return color(text, C.blue, bold);
}
function cyan(text: string, bold = false): string {
    return color(text, C.cyan, bold);
}
function magenta(text: string, bold = false): string {
...
function printUsage(): void {
    ...
    const headerLine = `${C.cyan}${C.bold}║${C.reset}  ${C.bold}${C.white}VulnPilot AI${C.reset} ${C.magenta}🛡️${C.reset}  ${C.bold}v${CLI_VERSION}${C.reset}       ${C.cyan}${C.bold}║${C.reset}`;
...
function formatConsoleReport(
    ...
    const parts: string[] = [];
    if (sev.critical > 0) parts.push(`${red(`● ${sev.critical} critical`, true)}`);
    if (sev.high > 0) parts.push(`${red(`● ${sev.high} high`)}`);
    if (sev.medium > 0) parts.push(`${yellow(`● ${sev.medium} medium`)}`);
    if (sev.low > 0) parts.push(`${blue(`● ${sev.low} low`)}`);
    if (parts.length === 0) {
    ...
    if (results.length > 0) {
        ...
        for (const result of results) {
            ...
            for (const finding of result.findings) {
                ...
                if (loc) lines.push(`      ${dim("Location:")} ${loc}`);
...
function main() {
    ...
    switch (options.command) {
        case "scan":
            ...
            break;
        case "version":
            console.log(`${green("✓")} ${CLI_NAME} ${cyan(CLI_VERSION)}`);
            break;
        case "doctor":
            runDoctor();
            break;
